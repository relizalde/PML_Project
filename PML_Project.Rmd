Predicting Quality of Weight Lifting Exercise
====================================
...
Also, we remove the columns with non-numeric variables such as user name, time stamps and window (Columns 1..6). These attributes are not generalizable for other data sets.
```{r}
...
I decided to try out Random Forests as the prediction algorithm. The model was train on the `trainSet` and then cross-validated on the `cv.testSet`.
...
modelFit <- train(trainSet$classe~., data = trainSet, method="rf", trControl = mControl)
modelFit
```
...
We use a Confusion Matrix in order to evaluate the accuracy of our model. Each column of the matrix represents the instances in a predicted class, while each row represents the instances in an actual (reference) class.
...
The following code was provided by instructor in order to generate submission files for grading.
```{r}
answers = as.character(testing.pred)
